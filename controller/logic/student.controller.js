...
exports.createStudent = (req, res, next) => {
	...
	studentDto.create(std, (err, data) =>{
		...
		let r = config.getStudend("roles").student;
		let user = {
			name: std.name,
		...
		userDto.create(user, (err, u) => {
			if(err){
				studentDto.delete({_id: data._id}, (e, data) =>{return res.status(400).json({error:err});});
			}
...
exports.updateStudent = (req, res, next) => {
	...
	studentDto.update({_id: res.body.id}, std, (err, data) =>{
		if(err){return res.status(400).json({error:err});}
		res.status(201).json({info:data});
	});
};
